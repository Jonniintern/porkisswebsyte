...
function before() {
  const imageArray = ["01", "02", "03", "04", "05", "06", "07", "08"];
  let randomImage = imageArray[Math.floor(Math.random() * imageArray.length)];

  document.getElementById("myImage").src = "./" + randomImage + "nft.png";
...
function playbeat() {
  ...
  audio.src = "./beats/" + randomImage + ".mp3";
  audio.play();
}

const windowlocker = document.getElementById("window");
function closewindow() {
  windowlocker.classList.add("hidden");
}
